...
- `!chat {prompt}` / `@jpi {prompt}` — Cloudflare Workers AI (`@cf/meta/llama-3.3-70b-instruct-fp8-fast`) でチャット応答。tool 呼び出し対応 (画像検索 / 抽選 / メッセージ削除)
...
- `src/ai/completions.ts` — `LlamaChat` クラス。Cloudflare Workers AI の `@cf/meta/llama-3.3-70b-instruct-fp8-fast` を叩く。`env.AI` (binding) を受け取り、`completions(prompt)` / `chat(messages)` で text 応答、`chatWithTools(messages, tools)` で tool calling 対応の自前ループ。
