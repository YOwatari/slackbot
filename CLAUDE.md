...
- `!chat {prompt}` — Cloudflare Workers AI (`@cf/meta/llama-4-scout-17b-16e-instruct`) で応答
...
- `src/ai/completions.ts` — `LlamaChat` クラス。Cloudflare Workers AI の `@cf/meta/llama-4-scout-17b-16e-instruct` を叩く。`env.AI` (binding) を受け取り、`completions(prompt)` で `{response}` または raw string 形式に対応した抽出を行う。
